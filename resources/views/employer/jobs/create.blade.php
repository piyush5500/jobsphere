...
.type-label:hover { border-color: #3498db; background: #f0f7ff; }
...
.status-opt:checked + .status-lbl.active-lbl {
    border-color: #27ae60;
    background: #e8f8f0;
    color: #27ae60;
}
.status-opt:checked + .status-lbl.inactive-lbl {
    border-color: #e74c3c;
    background: #fde8e8;
    color: #e74c3c;
}
...
<div class="post-job-wrap">
    ...
    <div class="pj-header">
        <div class="pj-header-inner">
            ...
            <p>Fill in the details below to publish your job listing and attract top candidates.</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('employer.jobs.store') }}">
        ...
        <div class="pj-card">
            <div class="pj-card-head">
                <div class="icon-wrap" style="background: linear-gradient(135deg,#2c3e50,#3498db);">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                </div>
                <div>
                    <h3>Basic Information</h3>
                    <p>Job title and full description</p>
                </div>
            </div>
            <div class="pj-card-body">
                <div class="pj-field">
                    <label class="pj-label">Job Title <span class="req">*</span></label>
                    <input type="text" name="title" class="pj-input" placeholder="e.g. Senior Software Engineer, Marketing Manager..." value="{{ old('title') }}" required maxlength="100">
                    <p class="pj-hint">Be specific — clear titles attract more qualified applicants</p>
                </div>
                <div class="pj-field">
                    <label class="pj-label">Job Description <span class="req">*</span></label>
                    <textarea name="description" class="pj-textarea" placeholder="Describe the role, responsibilities, requirements, and what makes this opportunity great..." required>{{ old('description') }}</textarea>
                    <p class="pj-hint">Include key responsibilities, required skills, experience level, and any benefits</p>
                </div>
            </div>
        </div>
        ...
        <div class="pj-card">
            <div class="pj-card-head">
                ...
                <div>
                    ...
                    <p>Where the job is and what it pays</p>
                </div>
            </div>
            <div class="pj-card-body">
                <div class="pj-row">
                    <div class="pj-field">
                        ...
                        <input type="text" name="location" class="pj-input" placeholder="e.g. New York, NY or Remote" value="{{ old('location') }}" required>
                    </div>
                    <div class="pj-field">
                        <label class="pj-label">Salary Range <span class="opt">(optional)</span></label>
                        <input type="text" name="salary" class="pj-input" placeholder="e.g. $80,000 – $120,000 / year" value="{{ old('salary') }}">
                        <p class="pj-hint">Listings with salary info get 30% more applicants</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="pj-card">
            <div class="pj-card-head">
                ...
                <div>
                    <h3>Job Type</h3>
                    <p>Select the employment type</p>
                </div>
            </div>
            <div class="pj-card-body">
                <div class="type-grid">
                    ...
                    <div>
                        <input type="radio" name="job_type" id="type_{{ $loop->index }}" value="{{ $type }}" class="type-option" {{ old('job_type')==$type ? 'checked' : ($loop->first && !old('job_type') ? 'checked' : '') }} required>
                        <label for="type_{{ $loop->index }}" class="type-label">
        ...
        <div class="pj-card">
            <div class="pj-card-head">
                ...
                <div>
                    ...
                    <p>Deadline and visibility options</p>
                </div>
            </div>
            <div class="pj-card-body">
                <div class="pj-row">
...
<p class="pj-hint">Select date only - future dates from today. Leave blank for no deadline.</p>
                    ...
                    <div class="pj-field">
                        ...
                        <div class="status-toggle">
                            <input type="radio" name="is_active" id="status_active" value="1" class="status-opt" {{ old('is_active','1')=='1' ? 'checked' : '' }}>
                            <label for="status_active" class="status-lbl active-lbl">✓ Active</label>
                            <input type="radio" name="is_active" id="status_inactive" value="0" class="status-opt" {{ old('is_active')=='0' ? 'checked' : '' }}>
                            <label for="status_inactive" class="status-lbl inactive-lbl">✗ Inactive</label>
                        </div>
                        <p class="pj-hint">Active listings accept applications immediately</p>
                    </div>
                </div>
            </div>
        </div>
